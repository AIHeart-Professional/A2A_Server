    ...
    async def execute(
        ...
        *,
        ...
        try:
            if stream:
                async for event in self.runner.run_async(
                    ...
                    if asyncio.get_event_loop().time() - start_time > timeout_seconds:
                        print(f"Agent execution timed out after {timeout_seconds} seconds")
            ...
            # Log timeout and re-raise - let the calling code handle it
            print(f"Agent execution timed out after {timeout_seconds} seconds")
            raise
        except Exception as e:
            # Log error and re-raise - let the calling code handle it
